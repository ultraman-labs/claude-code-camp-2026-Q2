...
class OpenAI(Base):
    ...
    def to_messages(self, context: Any) -> list[dict[str, Any]]:
        ...
        for msg in context.messages:
            if msg.role == "tool_result":
                messages.append({"role": "tool", "tool_call_id": msg.tool_use_id, "content": msg.content})
            else:
    ...
    def to_payload(self, context: Any, max_output_tokens: int = 1024) -> dict[str, Any]:
        return {"model": self.model, "messages": self.to_messages(context), "tools": self.to_tools(context.tools), "max_completion_tokens": max_output_tokens}
