...
class Gemini(Base):
    ...
    def to_payload(self, context: Any, max_output_tokens: int = 1024) -> dict[str, Any]:
        return {"systemInstruction": {"parts": [{"text": context.system}]}, "contents": self.to_messages(context), "tools": self.to_tools(context.tools), "generationConfig": {"maxOutputTokens": max_output_tokens}}
